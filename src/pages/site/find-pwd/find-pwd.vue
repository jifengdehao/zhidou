...
<template>
  <div class="form-wp">
    <mt-field type="tel" label="手机号" placeholder="请输入手机号" v-model.trim="params.mobile">
  ...
  export default {
    ...
    methods: {
      sendCheckCode() {
        ...
        if (!this.countdown) {
          if (!isMobile(mobile)) {
            ...
            this.API.forgetPasswordVerifyMobile(mobile).then((res) => {
              if (res.waitTime) {
      ...
      handleSubmit() {
        ...
        this.API.findPwd(params).then((res) => {
          if (res) {
            this.$toast('密码重置成功')
            setTimeout(() => {
